...
function getDecile(rank){
	let decile = Math.floor(parseInt(rank)/33755*10)+1
	if(decile>10){
		decile = 10;
	}
	return decile
...
function generateDataDownload(){
	let downloadData = [["Postcode", "Latitude", "Longitude", "Ward Code", "LA Code", "Local Authority", "IMD Decile", "Income Decile", "Employment Decile", "Education Decile", "Health and Disability Decile", "Crime Decile", "Housing and Services Decile", "Living Environment Decile"]];
	postcodeData.forEach(function(row){
		downloadData.push([row[0],row[1],row[2],row[3],row[4],row[5],getDecile(row[6]),getDecile(row[7]),getDecile(row[8]),getDecile(row[9]),getDecile(row[10]),getDecile(row[11]),getDecile(row[12]),getDecile(row[13])]);
	});
...
function init(){
	...
	$('#download-data').on('click',function(){
		console.log('trying to download map now')
